...
class TestClient(object):
    def test_construct_accesstoken_request(self):
        ...
        _context_1 = client_1.entity_get("service_context")
        ...
        _context2 = client_2.entity_get("service_context")
        _context2.load(_state_dump)

        auth_response = AuthorizationResponse(code='access_code')
        _context2.state.store_item(auth_response,'auth_response', _state)

        msg = client_2.entity_get("service",'accesstoken').construct(request_args={}, state=_state)
    ...
    def test_construct_refresh_token_request(self):
        ...
        client_1 = Client(config=CONF)
        _state = client_1.entity_get("service_context").state.create_state('issuer')
        ...
        client_1.entity_get("service_context").state.store_item(auth_request, 'auth_request', _state)

        # Client 2 carries on
        client_2 = Client(config=CONF)
        _state_dump = client_1.entity_get("service_context").dump()
        client_2.entity_get("service_context").load(_state_dump)

        auth_response = AuthorizationResponse(code='access_code')
        client_2.entity_get("service_context").state.store_item(auth_response, 'auth_response', _state)

        token_response = AccessTokenResponse(refresh_token="refresh_with_me",
                                             access_token="access")

        client_2.entity_get("service_context").state.store_item(token_response, 'token_response', _state)

        # Next up is Client 1
        _state_dump = client_2.entity_get("service_context").dump()
        client_1.entity_get("service_context").load(_state_dump)

        req_args = {}
        msg = client_1.entity_get("service",'refresh_token').construct(request_args=req_args, state=_state)
